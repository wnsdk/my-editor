 ...
 * @param {HTMLElement} el - 캐럿 오프셋을 계산할 대상 요소.
 * @returns {number} 요소 내 캐럿의 텍스트 오프셋. 선택 영역이 없으면 0을 반환합니다.
 */
export function getCaretOffsetWithin(el) {
 ...
 * @returns {{left: number, top: number}|null} 캐럿의 좌표 객체 또는 선택 영역이 없으면 null.
 */
export function getCaretCoordinates() {
    ...
    if (range.getClientRects().length > 0) {
        ...
        if (range.startOffset === 0 && range.startContainer.textContent.length > 0) {
